...
const MarqueeRow = ({ items, direction = 1, speed = 40 }: any) => {
  return (
    // Transform specifically optimized here
    <div className="flex w-full overflow-hidden whitespace-nowrap">
      <motion.div
        animate={{ x: direction > 0 ? ["0%", "-50%"] : ["-50%", "0%"] }}
        transition={{ ease: "linear", duration: speed, repeat: Infinity }}
        className="flex gap-4 md:gap-6 px-2 md:px-3 w-fit will-change-transform"
      >
        {[...items, ...items, ...items, ...items].map((item, i) => (
          <div key={i} className="group relative flex items-center gap-3 rounded-full border border-white/10 bg-[#111] md:bg-[#050505]/50 px-5 py-2.5 transition-all duration-300 hover:border-neon hover:bg-neon/5">
            <div className="relative h-5 w-5 md:h-6 md:w-6 flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
              <img src={item.icon} alt={item.name} className="h-full w-full object-contain drop-shadow-md" loading="lazy" />
            </div>
            <span className="whitespace-nowrap font-mono font-medium text-sm md:text-base text-white/80 group-hover:text-white transition-colors">{item.name}</span>
          </div>
        ))}
      </motion.div>
    </div>
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        className="relative flex flex-col gap-6 py-10 w-full mt-10"
      >
