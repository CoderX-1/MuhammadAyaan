...
const TestimonialCard: React.FC<{ review: typeof REVIEWS[0] }> = ({ review }) => {
  return (
    <div className="flex-shrink-0 w-[85vw] md:w-[500px] h-[350px] md:h-[400px] bg-white/[0.03] border border-white/10 p-8 md:p-12 flex flex-col justify-between group transition-all duration-500 hover:bg-white/[0.08] hover:border-white/20 mx-4 md:mx-6 rounded-2xl">
...
const Testimonials: React.FC = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (row1Ref.current) {
        const rowWidth = row1Ref.current.offsetWidth;
